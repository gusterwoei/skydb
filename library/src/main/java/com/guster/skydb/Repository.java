               ...
               switch (type) {
                  ...
                  case Cursor.FIELD_TYPE_STRING:
                     if (field.getType().equals(Boolean.class) || field.getType().equals(Boolean.TYPE)) {
                        ...
                        // extra check in case if the field type is Long and the value is null
                        val = cursor.getLong(columnIndex);
                     } else if (field.getType().equals(Integer.class) || field.getType().equals(Integer.TYPE)) {
                        // extra check in case if the field type is Integer and the value is null
                        val = cursor.getInt(columnIndex);
                     } else {
   ...
   protected ContentValues getFields(final T t) {
      ...
      forEachDbField(t, new OnEachFieldListener() {
         ...
         public void onEachField(String column, Object value, Field field, Column dbField, int index) {
            ...
            // auto increment for UUID generation
            if (dbField.autoIncrement() && dbField.incrementRule().equals(Rule.UUID) && (value == null || "".equals(value))) {
               String uuid = UUID.randomUUID().toString();
               values.put(column, uuid);

               // add back uuid to the entity itself
               try {
                  field.set(t, uuid);
               } catch (IllegalAccessException e) {
                  throw new SkyDbException(e);
