...
public abstract class SkyDatabase extends SQLiteOpenHelper {
   ...
   private String constructColumnStmnt(Class fieldType, Column col, List<String> uniqueColumns) {
      // validations
      if(col.autoIncrement() && col.incrementRule().equals(Rule.UUID) && !fieldType.equals(String.class))
         throw new SkyDbException("UUID incremental rule can be only be applied to String type");
      ...
      // auto increment?
      if (col.autoIncrement() && col.incrementRule().equals(Rule.INTEGER))
         str += "AUTOINCREMENT ";
